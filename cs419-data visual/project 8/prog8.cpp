...
t0 = glutGet( GLUT_ELAPSED_TIME );
	...
	//printf("scale: %f took this much time: %f\n", VectorScaler,difftime (end,start));

t1 = glutGet( GLUT_ELAPSED_TIME );
glutSwapBuffers();
//fprintf( stderr, "%d display frame took %d milliseconds at vectorsize: %f \n",x, t1 - t0,VectorScaler );
ave += (t1-t0);
fprintf( stderr, "%d\n",x);
//fprintf( stderr, "%d\n",t1 - t0);
//fprintf( stderr, "%f\n",VectorScaler );
}
printf("average milliseconds %d\n", (ave/100));
}
else
{
	VectorScaler = .01;
	...
	InitGlui( void )
{
	...
	Glui->add_checkbox( (char*)"Vector Cloud", &VectorCloud );
	Glui->add_checkbox( (char*)"Test", &test );
