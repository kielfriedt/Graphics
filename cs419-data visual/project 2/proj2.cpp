...
//
//
>>>>>>> 5bc840dc6806a9c50e67e1bd620b1871ea13b979
...
GLuint    PointList;        // object display list
...
<<<<<<< HEAD
	...
	//3d array node struct
	Node = new node**[NX];
	for (int i = 0; i < NX; i++) {
		Node[i] = new node*[NY];
		for (int j = 0; j < NZ; j++)
			Node[i][j] = new node[NZ];
	}
...
//
// Temperature function that creates the data as to where the hot 
// spots are within the point cloud.
//
...
// setup glui windows with all additonal UI items
//
void
InitGlui( void )
{
    ...
    for(int i = 0; i &lt; NX; i++){
        ...
        for(int j = 0; j &lt; NY; j++){
            ...
            for(int k = 0; k &lt; NZ; k++){
                ...
                Node[i][j][k].z = z;
                x -= .0625;
                Node[i][j][k].t = Temperature(Node[i][j][k].x, Node[i][j][k].y, Node[i][j][k].z);
            }
            y -= .0625;
        }
        z -= .0625;
...
// Creates point cloud and changes value of grey based on node location within the cloud
//
void
InitLists( void )
{
    ...
    glPointSize( 1 );    // hardwire this, or set it with a spinner  //# of pixels
    glBegin( GL_POINTS );
    for( x = 0; x &lt; NX; x++ )
    for( y = 0; y &lt; NY; y++ )
    for( z = 0; z &lt; NZ; z++ )
    {
        gray = GRAYMIN + ( GRAYMAX - GRAYMIN ) * ( Node[x][y][z].t - TEMPMIN ) / ( TEMPMAX - TEMPMIN );
        glBegin(GL_POINTS);
        glColor3f( gray, gray, gray );
        glVertex3f(Node[x][y][z].x,  Node[x][y][z].y, Node[x][y][z].z );
    }
    glEnd();
    glEndList( );
