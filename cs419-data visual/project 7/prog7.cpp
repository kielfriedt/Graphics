...
const char *WINDOWTITLE = { "Project 6 -- KIEL FRIEDT" };
...
int CUBEROOTOFNODES = 14;
const float COORDRANGE = 2;

const float VCSMIN = {   0.f };
const float VCSMAX = { 100.f };

const float FSRMIN = { 0.20f };
const float FSRMAX = { 1.00f };
...
#define FSR		0
const char *	FSRFORMAT = { "Flow Speed Range: %5.2f - %5.2f" };
float			FSRLowHigh[2];
GLUI_HSlider *		FSRSlider;
GLUI_StaticText *	FSRLabel;

#define CR	1
const char *	CRFORMAT = { "Curl Range: %5.2f - %5.2f" };
float			 CRLowHigh[2];
GLUI_HSlider *		CRSlider;
GLUI_StaticText *	CRLabel;

// structure declaration
struct node
{
	float x, y, z;          // location
	float vx, vy, vz;         // velocity at location
...
int	WhichProbe;		// index into Colors[]
...
void RibbonTrace(float x, float y, float z)
{
	float origx =x;
	float origy =y;
	float origz =z;
	bool out = false;
	float  x2, y2, z2, vx, vy, vz, vx2, vy2, vz2;
